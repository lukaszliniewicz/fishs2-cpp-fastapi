...
INSTRUCTION_FISH_FIELDS = set(FishS2Params.model_fields.keys())

app = FastAPI(
    title="FishS2 FastAPI Server",
...
def _parse_instruction_overrides(instructions: str | None) -> dict:
    ...
    try:
        ...
        raise APIError(
            "instructions must be valid JSON when used for FishS2 overrides",
            param="instructions",
    ...
    fish_overrides: dict[str, object] = {}
    raw_fish = payload.get("fishs2")
    if raw_fish is not None:
        if not isinstance(raw_fish, dict):
            raise APIError(
                "instructions.fishs2 must be a JSON object",
                param="instructions",
                code="invalid_instructions_fishs2",
                status=422,
            )
        fish_overrides.update(raw_fish)

    for key in INSTRUCTION_FISH_FIELDS:
        if key in payload:
            fish_overrides[key] = payload[key]

    scalar_fields: dict[str, object] = {}
    for key in ("prompt_text", "reference_text", "control"):
        if key in payload:
    ...
    reference_text = scalar_fields.get("reference_text")
    if reference_text is not None and not isinstance(reference_text, str):
        raise APIError(
            "instructions.reference_text must be a string",
            param="instructions",
            code="invalid_instructions_reference_text",
            status=422,
        )
...
def _apply_instruction_overrides(body: CreateSpeechRequest) -> CreateSpeechRequest:
    ...
    fish_overrides = overrides.get("fishs2")
    if fish_overrides is not None:
        existing_fish = {}
        if body.fishs2 is not None:
            existing_fish = body.fishs2.model_dump(exclude_none=True)
        merged_fish = {**fish_overrides, **existing_fish}
        try:
            payload["fishs2"] = FishS2Params.model_validate(merged_fish)
        except ValidationError as exc:
            details = exc.errors(include_url=False)
            message = details[0]["msg"] if details else str(exc)
            raise APIError(
                f"Invalid FishS2 overrides in instructions: {message}",
                param="instructions",
                code="invalid_instructions_fishs2",
                status=422,
            ) from exc
...
async def create_voice(
    ...
    prompt_text: str | None = Form(default=None, description="Prompt transcript for cloning"),
):
    _ = purpose
...
async def create_speech(body: CreateSpeechRequest):
    ...
    except S2RuntimeUnavailable as exc:
        raise APIError(
            str(exc),
