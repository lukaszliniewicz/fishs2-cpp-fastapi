...
APP_LOGGER_NAME = "fishs2_fastapi.app"
ACCESS_LOGGER_NAME = "fishs2_fastapi.access"
ERROR_LOGGER_NAME = "fishs2_fastapi.errors"
...
def _logger_has_marker(logger: logging.Logger, marker: str) -> bool:
    for handler in logger.handlers:
        if getattr(handler, "_fishs2_marker", None) == marker:
            return True
    return False
...
def configure_file_logging(
    *,
    ...
    if app_handler is not None:
        root_logger = logging.getLogger("fishs2_fastapi")
        root_logger.setLevel(base_level)
        if not _logger_has_marker(root_logger, "fishs2_app_file"):
        ...
        if not _logger_has_marker(service_app_logger, "fishs2_app_file"):
            service_app_logger.addHandler(app_handler)
            configured_any = True

        uvicorn_error_logger = logging.getLogger("uvicorn.error")
        uvicorn_error_logger.setLevel(base_level)
        if not _logger_has_marker(uvicorn_error_logger, "fishs2_app_file"):
    ...
    if access_handler is not None:
        ...
        if not _logger_has_marker(access_logger, "fishs2_access_file"):
            access_logger.addHandler(access_handler)
            configured_any = True

        uvicorn_access_logger = logging.getLogger("uvicorn.access")
        uvicorn_access_logger.setLevel(logging.INFO)
        if not _logger_has_marker(uvicorn_access_logger, "fishs2_access_file"):
    ...
    if error_handler is not None:
        ...
        if not _logger_has_marker(error_logger, "fishs2_error_file"):
            error_logger.addHandler(error_handler)
            configured_any = True

        uvicorn_error_logger = logging.getLogger("uvicorn.error")
        if not _logger_has_marker(uvicorn_error_logger, "fishs2_error_file"):
